...
const API_CONFIG = {
    ...
    BASE_URL: 'https://sinpo.id/storage/data',
...
const API = {
    ...
    berita: {
       /* list: (params = {}) => {
            ...
            return apiRequest(`/berita60.json?${query}`, { type: 'news' });
        },*/
    ...
    list: (params = {}) => {
        ...
        // Kalau ada channel → pakai file channel
        if (channel) {
            return apiRequest(`/channel-${channel}.json?${query}`, { type: 'news' });
        }
    ...
    return apiRequest(`/berita/${folder}/${id}.json`, { type: 'news' });
},
        headline: (params = {}) => {
            const query = new URLSearchParams(params).toString();
            return apiRequest(`/headline.json?${query}`, { type: 'news' });
        },
        populer: (params = {}) => {
            const query = new URLSearchParams(params).toString();
            return apiRequest(`/populer.json?${query}`, { type: 'news' });
        },
        terkait: (id, params = {}) => {
            const query = new URLSearchParams(params).toString();
    ...
    kategori: {
        list: (params = {}) => {
            const query = new URLSearchParams(params).toString();
            return apiRequest(`/kategori.json?${query}`, { type: 'category' });
        },
    ...
    channel: {
        list: (params = {}) => {
            const query = new URLSearchParams(params).toString();
            return apiRequest(`/channel.json?${query}`, { type: 'channel' });
        },
...
function navigate(page, event, params = {}) {
    ...
    if (page === 'home') {
        ...
        //window.location.hash = `article/${params.id}`;
         ...
         const slug = params.title || 'artikel';
        const url = `/detail/${params.id}/${slug}`;
        window.history.pushState({ page, params }, '', url);

    } else if (page === 'category' && params.id) {
       // window.location.hash = `category/${params.id}`;

        const url = `/kategori/${params.id}`;
    window.history.pushState({ page, params }, '', url);
    ...
    } else if (page === 'tag' && params.tag) {

    const url = `/berita?tag=${encodeURIComponent(params.tag)}`;
    window.history.pushState({ page, params }, '', url);
}
    ...
    return `
        <section class="hero">
            ...
            <div class="hero-content">
                ...
                <h1 class="hero-title" onclick="navigate('article', event, {id: ${headline.id},title: '${createSlug(headline.title)}'})">
...
function createSidebarCard(news, showCategory = true) {
    ...
    return `
        <div class="sidebar-news-card" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'})">
            ${categoryHtml}
...
function createSidebarCard2(news, showCategory = true) {
    const categoryName = formatCategoryName(news.category);
    const catClass = categoryName.toLowerCase() === 'sin po dulu' ? 'cat-large-red' : 'cat-small';
    const categoryHtml = showCategory ? `<div class="${catClass}">${escapeHtml(categoryName)}</div>` : '';
...
function updateSEO({
    ...
    // keywords
    let keywordTag = document.querySelector(
        'meta[name="keywords"]'
    );

    if (keywordTag) {
        keywordTag.setAttribute(
            'content',
            keywords
        );
    }
...
async function updateArticleCounter(articleId) {
...
async function renderArticle(id) {
    const [detailRes, sidebarRes, bongkarRes] = await Promise.all([
    ...
    title:
        article.title + ' - SinPo.id',

    description:article.summary,

    canonical:
        `https://sinpo.id/detail/${article.id}/${createSlug(article.title)}`,
...
const latestNews = safeArray(latestRes.data)
    .filter(item => item.id !== article.id)
    .slice(0, 5);
  ...
  //  const comments = safeArray(commentsRes.data);
    ...
    updateArticleCounter(article.id);
    const app = document.getElementById('app');
    app.innerHTML = `
                ...
                <div class="article-main-column">
                    <article class="article-detail">
                        <h1 class="article-title">${escapeHtml(article.title)}</h1>
                         ...
                         <div>
                                <div class="share-box">
    <span class="share-label publisher-value">Bagikan:</span>
    ...
    <button class="share-btn telegram" onclick="shareArticle('telegram')">
        TG
    </button>
                        ...
                        <div class="publisher-info-bar desktop-only">
                            ...
                            <div class="publisher-group">
                                <div class="publisher-details align-right">
                                    ...
                                    <span class="publisher-value" id="counter">${(article.views || 0).toLocaleString('id-ID')} KALI</span>
                                </div>
                            </div>
                        </div>
            ...
            ${article.gallery.map(item => `
                <div class="gallery-slide">
                    ...
                    <img
                        ...
                        alt="${escapeHtml(item.title || article.title)}"
                        loading="lazy"
                        onclick="openGalleryImage('${getImageUrl(item.image)}')">
...
` : `<img src="${getImageUrl(article.image || article.cover)}" 
    ...
    <span 
        ...
        onclick="navigate('tag', event, {tag: '${tag.replace(/'/g, "\\'")}'})"
    >
        #${escapeHtml(tag)}
    </span>
`).join('')}
                ...
                <div class="category-sidebar-col">
                    ...
                    <div class="cat-sidebar-list" id="category-sidebar-list">
                        ...
                        <div class="latest-news-section" style="margin-top:40px">

        ${latestNews.map(news => createTimelineItem(news)).join('')}
                ...
                <aside class="article-sidebar">
                    ...
                    ${populerNews.length > 0 ? `
                        <div class="popular-section-new" style="margin-top: 3rem;">
                            ...
                            <!-- Featured Item (First) -->
                            <div class="populer-featured" onclick="navigate('article', event, {id: ${populerNews[0].id},title: '${createSlug(populerNews[0].title)}'})">
                            ...
                            <div class="populer-list-items">
                                ${populerNews.slice(1).map(news => `
                                    <div class="populer-list-row" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'})">
...
async function renderTag(tagName) {
    ...
    description:
        'Kumpulan berita dengan tag ' +
        tagName,

    canonical:
        `https://sinpo.id/berita?tag=${encodeURIComponent(tagName)}`
    ...
    try {
        ...
        const allNews = safeArray(res.data);
        ...
        app.innerHTML = `
            <div class="container">
                ...
                ${
                    ...
                    ? `
                        <div class="news-grid">
                            ${filteredNews.map(item => createNewsCard(item)).join('')}
                        </div>
                    `
                    : `
                        <div class="empty-state text-center">
                            Tidak ada artikel dengan tag "${escapeHtml(tagName)}"
                        </div>
                    `
                }
        ...
        showError('Gagal memuat halaman tag');
    }
}
...
async function renderCategory(categoryId) {
    ...
    //document.title ='Kategori ' + categoryName + ' - SinPo.id';
    ...
    canonical:
        `https://sinpo.id/kategori/${categoryId}`
    ...
    app.innerHTML = `
        <div class="container">
            <h1 class="category-label" style="margin-top:20px">
    ...
    try {
        ...
        if (news.length > 0) {
            ...
            heroMainEl.innerHTML = `
                <h2 class="cat-overlay-title" onclick="navigate('article', event, {id: ${heroItem.id},title: '${createSlug(heroItem.title)}'})">${escapeHtml(heroItem.title)}</h2>
            ...
            if (news.length > 1) {
                ...
                listItems.forEach((item, index) => {
                    listEl.innerHTML += `
                        <div class="cat-overlay-list-item" onclick="navigate('article', event, {id: ${item.id},title: '${createSlug(item.title)}'})">
            ...
            if (news.length > 5) {
                ...
                highFeatureEl.innerHTML = `
                    <div class="highlight-feature-img-wrapper" onclick="navigate('article', event, {id: ${featItem.id},title: '${createSlug(featItem.title)}'})">
                    ...
                    <div class="highlight-feature-content">
                        ...
                        <h3 onclick="navigate('article', event, {id: ${featItem.id},title: '${createSlug(featItem.title)}'})">${escapeHtml(featItem.title)}</h3>
                    </div>
                `;
                ...
                if (highItems.length > 1) {
                    ...
                    sideItems.forEach(item => {
                        highListEl.innerHTML += `
                            <div class="highlight-list-card" onclick="navigate('article', event, {id: ${item.id},title: '${createSlug(item.title)}'})">
            ...
            if (news.length > 15) {
                ...
                sidebarItems.forEach(item => {
                    sidebarListEl.innerHTML += `
                        <div class="cat-sidebar-item" onclick="navigate('article', event, {id: ${item.id},title: '${createSlug(item.title)}'})">
            ...
            registerSeenNews(news.slice(0, 20));
...
function createTimelineItem(news) {
    ...
    return `
        <div class="timeline-item" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'})">
...
function createTimelineItem2(news) {

    const newsId = news.id_berita;
    const title = news.judul || '';
    const image = news.gambar_detail || '';
    const channel = news.datachannel?.nama || '';
    const author = news.datawartawan?.nama_wartawan || news.wartawan || 'Tim Redaksi';

    const publishedDate =
        news.tanggal_tayang && news.waktu
            ? `${news.tanggal_tayang} ${news.waktu}`
            : news.tanggal_tayang;

    return `
        <div class="timeline-item"
            onclick="navigate('article', event, {
                id: ${newsId},
...
function createNewsCard(news) {
    return `
        <article class="news-card" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'})">
            <img src="${getImageUrl(news.image || news.cover)}" 
                 alt="${escapeHtml(news.title)}" 
...
function createGalleryCard(gallery) {
    return `
        <div class="gallery-card" onclick="${gallery.article_id ? `navigate('article', event, {id: ${gallery.article_id},title: '${createSlug(gallery.title)}'})` : ''}">
...
async function renderIndeks() {

    updateSEO({

    title:
        'Indeks Berita - SinPo.id',

    description:
        'Indeks berita harian SinPo.id',

    canonical:
        'https://sinpo.id/indeks'
    ...
    const [sidebarRes,bongkarRes] = await Promise.all([
        API.berita.populer({ limit: 7 }),
        API.berita.list({ channel: 21, limit: 10 }).catch(() => ({ data: [] }))
    ]);
    ...
    app.innerHTML = `
        <div class="article-wrapper">
            <div class="container article-container-grid">
                ...
                <div class="article-main-column">
                    <article class="article-detail">
                        <h1 class="article-title">INDEKS</h1>

                        <div class="article-content">
                             <div id="calendar-container"></div>
                             <h1 id="index-date-title"></h1>
                             <div id="index-news-container"></div>
                        </div>
...
async function loadIndexNews(ymd){
    ...
    throw new Error(`HTTP ${response.status}`);
        ...
        document.getElementById(
            ...
            .map(item => createTimelineItem2(item))
            .join('');

        document.getElementById(
            'index-date-title'
        ).innerHTML = formatIndexDate(ymd);
        ...
        document.getElementById(
            ...
            <div class="empty-state">
                Tidak ada berita pada tanggal ini
            </div>
        `;
    }
}

function formatIndexDate(ymd) {
...
const selectedDate = `${year}-${month}-${day}`;
    ...
    document
        .getElementById('index-date-picker')
        .addEventListener('change', function(){

            const selected =
                this.value.replaceAll('-','');

            loadIndexNews(selected);
        });
}
...
async function renderHome() {
    ...
    updateSEO({
    title: 'SinPo.id - Politik dan Peristiwa Terkini',
    description:
        'Berita politik, nasional, ekonomi, hukum dan peristiwa terkini dari Indonesia.',
    canonical:
        'https://sinpo.id/'
});
    ...
    try {
        ...
        const [headlineRes, newsPoolRes, popularRes, bongkarRes, pojokRes, hukumRes, politikRes] = await Promise.all([
            API.berita.headline({ limit: 1 }).catch(() => ({ data: [] })),
            API.berita.list({ limit: 60 }).catch(() => ({ data: [] })),
            API.berita.populer({ limit: 15 }).catch(() => ({ data: [] })),
            API.berita.list({ channel: 21, limit: 10 }).catch(() => ({ data: [] })),
            API.berita.list({ channel: 24, limit: 2 }).catch(() => ({ data: [] })),
            API.berita.list({ channel: 3, limit: 5 }).catch(() => ({ data: [] })),
            API.berita.list({ channel: 2, limit: 5 }).catch(() => ({ data: [] }))
        ]);
        ...
        const bongkarNews = safeArray(bongkarRes?.data);
        const pojokNews = safeArray(pojokRes?.data);
        const hukumNews = safeArray(hukumRes?.data).slice(0, 5);
        const politikNews = safeArray(politikRes?.data).slice(0, 5);
        ...
        const timelineNews = getNextUnique(10);
        ...
        const sidebarNews = bongkarNews.slice(0, 1);
        const pojok2News = pojokNews.slice(0, 1);
        const mobilePopularNews = bongkarNews.slice(0, 5); // Use Bongkar for mobile popularBERIT UTAMA
       ...
       /* if (timelineNews.length >= 5) {
        ...
        if (sidebarNews.length > 0) {
          ...
          //  html += `<div class="sidebar-underline"></div>`;
            ...
            html += `<div style="padding-bottom:30px"></div>`;
             pojok2News.forEach((news, index) => {
                if (index > 0) html += `<div class="sidebar-divider"></div>`;
                if (typeof createSidebarCard2 === 'function') html += createSidebarCard2(news);
            });
        ...
        if (trendingNews.length > 0) {
          ...
          //  html += createSinPoTVSection();
            ...
            html += createBeritaUtamaSection(beritaUtamaNews,hukumNews,politikNews);
        }
...
function createFeaturedMain(news) {
    ...
    return `
        <div class="featured-main" onclick="navigate('article', event, {id: ${news.id},title: '${slug}'})">
            <img src="${getImageUrl(news.image || news.cover)}" 
                 alt="${escapeHtml(news.title)}" 
...
function createFeaturedSecondary(news) {
    const slug = createSlug(news.title);
    return `
        <div class="featured-secondary" onclick="navigate('article', event, {id: ${news.id},title: '${slug}'})">
            <img src="${getImageUrl(news.image || news.cover)}" 
                 alt="${escapeHtml(news.title)}" 
...
function createRelatedFeatured(news) {
    const slug = createSlug(news.title);
    return `
        <div class="related-featured" onclick="navigate('article', event, {id: ${news.id},title: '${slug}'})">
...
function createRelatedText(news) {
    const slug = createSlug(news.title);
    return `
        <div class="related-text-item" onclick="navigate('article', event, {id: ${news.id},title: '${slug}'})">
...
function createFeaturedListItem(news) {
    const slug = createSlug(news.title);
    return `
        <div class="featured-list-item" onclick="navigate('article', event, {id: ${news.id},title: '${slug}'})">
            <img src="${getImageUrl(news.image || news.cover)}" 
                 alt="${escapeHtml(news.title)}" 
...
async function renderSearch(query) {
    ...
    if (recsItems.length > 0) {
        ...
        recsItems.forEach((item, index) => {
            ...
            recsList.innerHTML += `
                <div class="recommendation-item" onclick="navigate('article', event, {id: ${item.id},title: '${slug}'})">
        ...
        if (latestContainer) {
            if (latestItems.length > 0) {
                latestContainer.innerHTML = latestItems.map(news => `
                    <div class="sp-news-item" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'}); toggleSidePanel();">
                        <img src="${getImageUrl(news.image || news.cover)}" 
                             alt="${escapeHtml(news.title)}" 
                             class="sp-news-thumb"
                             loading="lazy"
        ...
        if (popularContainer) {
            if (popularItems.length > 0) {
                popularContainer.innerHTML = popularItems.map((news, index) => `
                    <div class="sp-popular-item" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'}); toggleSidePanel();">
            ...
            <div class="trending-list" id="trending-scroll-container">
                ${chunks.map((chunk, pageIdx) => `
                    <div class="trending-page" data-page="${pageIdx}">
                        ${chunk.map(news => `
                            <div class="trending-item" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'})">
                                <img src="${getImageUrl(news.image || news.cover)}" 
                                     alt="${escapeHtml(news.title)}" 
...
function createMobilePopularItem(news, number, showCategory = true) {
    ...
    return `
        <div class="mobile-popular-item" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'})">
...
// ==========================================
function createBeritaUtamaSection(newsList,hukumNews,politikNews) {
    if (!newsList || newsList.length === 0) return '';
                ...
                <div class="berita-utama-grid">
                    ...
                    <div class="berita-utama-right">
                        <!-- Placeholder for future content -->
                          ...
                          <div class="popular-section-new" style="margin: 2rem;">
                            <h3 class="sidebar-title-strong">HUKUM</h3>
                        ...
                        <div class="popular-section-new" style="margin: 2rem;">
                            <h3 class="sidebar-title-strong">POLITIK</h3>
...
function createBeritaUtamaItem(news, showDescription = false) {
    ...
    return `
        <div class="berita-utama-item" onclick="navigate('article', event, {id: ${news.id},title: '${createSlug(news.title)}'})">
    ...
    try {
       ...
       // console.log(res);
        ...
        const rawItems = safeArray(res.data);
         ...
         // console.log('rawItems', rawItems.length);


        const newItems = filterSeenNews(rawItems).slice(0, 12); // Take only 12 unique

      //  console.log('after filter', filterSeenNews(rawItems).length);
       // alert(newItems.length);
        if (newItems.length > 0) {
           // registerSeenNews(newItems.length);
            registerSeenNews(newItems);
            newItems.forEach(item => {
